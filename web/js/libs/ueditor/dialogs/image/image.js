        ...
        for (var j = 0, length = tabBodys.length; j < length; j++) {
            ...
            if (id != tabSrc) {
                ...
                if (id == "imgManager") {
                    list.style.display = "";
                    //已经初始化过时不再重复提交请求
                    var dourl = parent.document.getElementById('aaa').value;
                    //alert(dourl);
                    if (!list.children.length) {
                        ajax.request(dourl, {
                            ...
                            onsuccess:function (xhr) {
                            	var vv = xhr.responseText;
                            	vv = vv.replace(new RegExp("\\\\/","gm"),"/");
                            	
                            	
                            	var vvv = JSON.stringify(vv);
                            	
                            	if(vvv.length > 10){
                            		vvv =vvv.substring(13, vvv.length);
                            		vvv = vvv.substring(0, vvv.length - 4);
                            	}
                            	
                                //去除空格
                                var tmp = utils.trim(vvv),
